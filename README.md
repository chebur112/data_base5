# Отчет по проектированию информационной модели для реляционной БД "Магазины"
...
```
erDiagram
    SHOP ||--o{ INVENTORY : "имеет"
    PRODUCT ||--o{ INVENTORY : "продается"

    SHOP {
        integer shop_number PK "Номер магазина"
    ...
    PRODUCT {
        integer product_id PK "ID товара"
        string product_name "Наименование товара"
        string variety "Сорт"
    }

    INVENTORY {
        integer inventory_id PK "ID записи инвентаря"
        integer shop_number FK "Номер магазина"
        integer product_id FK "ID товара"
        string unit_of_measure "Единица измерения"
        decimal unit_price "Цена за единицу"
        decimal quantity "Количество единиц"
    }
```
...
erDiagram
    SHOP {
        integer shop_number PK "Первичный ключ"
        varchar shop_name "Наименование (обязательно)"
        varchar address "Адрес"
        numeric area "Площадь (м²)"
    }

    PRODUCT {
        integer product_id PK "Первичный ключ"
        varchar product_name "Наименование (обязательно)"
        varchar variety "Сорт"
    }

    INVENTORY {
        integer inventory_id PK "Первичный ключ"
        integer shop_number FK "Ссылка на магазин"
        integer product_id FK "Ссылка на товар"
        varchar unit_of_measure "Единица измерения"
        numeric unit_price "Цена за единицу"
        numeric quantity "Количество единиц"
    }

    SHOP ||--o{ INVENTORY : "в магазине продается"
    PRODUCT ||--o{ INVENTORY : "товар в инвентаре"
```
...
erDiagram
    SHOP {
        integer shop_number PK "PRIMARY KEY"
        varchar shop_name "NOT NULL, UNIQUE"
        varchar address "NOT NULL"
        numeric area "NOT NULL CHECK (area > 0)"
    }

    PRODUCT {
        integer product_id PK "SERIAL PRIMARY KEY"
        varchar product_name "NOT NULL"
        varchar variety "NULL"
        unique "UNIQUE(product_name, variety)"
    }

    INVENTORY {
        integer inventory_id PK "SERIAL PRIMARY KEY"
        integer shop_number FK "NOT NULL REFERENCES Shop(shop_number)"
        integer product_id FK "NOT NULL REFERENCES Product(product_id)"
        varchar unit_of_measure "NOT NULL"
        numeric unit_price "NOT NULL CHECK (unit_price > 0)"
        numeric quantity "NOT NULL CHECK (quantity >= 0)"
        unique "UNIQUE(shop_number, product_id)"
    }

    SHOP ||--o{ INVENTORY : "FOREIGN KEY (shop_number)"
    PRODUCT ||--o{ INVENTORY : "FOREIGN KEY (product_id)"
```
